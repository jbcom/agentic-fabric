...
class TestCLIEntryPoint:
    """Smoke tests for the agentic-fabric CLI."""

    def test_no_args_prints_help(self, capsys) -> None:
        """Running with no args should print help text."""
        with patch("sys.argv", ["agentic-fabric"]):
            main()
        captured = capsys.readouterr()
        assert "agentic-fabric" in captured.out.lower() or "usage" in captured.out.lower()

    def test_help_flag(self) -> None:
        """--help should exit 0."""
        with (
            patch("sys.argv", ["agentic-fabric", "--help"]),
...
class TestListRunnersCommand:
    ...
    def test_list_runners_command(self, capsys) -> None:
        ...
        with patch("sys.argv", ["agentic-fabric", "list-runners"]):
            main()
        captured = capsys.readouterr()
