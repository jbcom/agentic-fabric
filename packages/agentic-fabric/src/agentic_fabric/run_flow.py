...
def run_tdd_prototype(requirements: dict[str, Any]) -> Any:
    ...
    flow_cls = _load_flow("agentic_fabric.flows.tdd_prototype_flow", "TDDPrototypeFlow")
    flow = flow_cls()
...
def run_meshy_asset(species: str, prompt: str, retexture_prompt: str) -> Any:
    ...
    flow_cls = _load_flow("agentic_fabric.flows.meshy_asset_flow", "MeshyAssetFlow")
    flow = flow_cls()
...
def run_prototype_assessment(prototypes: list[Any]) -> Any:
    ...
    flow_cls = _load_flow(
        "agentic_fabric.flows.prototype_to_production_flow",
...
def run_asset_integration(asset_manifest: dict[str, Any]) -> Any:
    ...
    flow_cls = _load_flow("agentic_fabric.flows.asset_integration_flow", "AssetIntegrationFlow")
    flow = flow_cls()
...
def run_hitl_review(content_type: str, content_url: str) -> Any:
    ...
    flow_cls = _load_flow("agentic_fabric.flows.hitl_review_flow", "HITLReviewFlow")
    flow = flow_cls()
...
def run_batch_generation(species_list: list[str]) -> Any:
    ...
    flow_cls = _load_flow("agentic_fabric.flows.batch_generation_flow", "BatchGenerationFlow")
    flow = flow_cls()
